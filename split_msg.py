...
@click.command()
@click.option('--max-len', default=4096, help='One fragment length (default: 4096)')
@click.argument('message_source')
def main(message_source: str, max_len: int):
    """
    Processes a message file, ensuring it exists and performs operations based on the provided maximum fragment length.
    ...
    if not os.path.isfile(message_source):
        raise FileNotFoundError(f"The file at path '{message_source}' does not exist.")
